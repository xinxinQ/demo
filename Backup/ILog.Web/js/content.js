...
$(document).ready(function() {
    ...
    //获取原创信息
    GetContentInfoById("" + vServiceUrl + "VipIlogUser.asmx/IlogGetContentInfoById", ioid, actionType);
...
function VipILogPersonalInfo(servicesUrl, userid) {
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            //设置title
            ShowTitie(dataObj.nickname + "的iLog");
            ...
            var certificationHtml = ShowVerifyImg(dataObj.vi_memberlevel);
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (dataObj.UrlState == 1) {
                ...
                showCount += "<strong class=\" Fw\"><a href=\"/h\">" + dataObj.blog + "</a></strong><br>";
                ...
                if (item.State != 0) {
                    if (idx == 0) {
                        if (item.State == 1) {
                            ...
                            original += "<div class=\"Round3\" style=\"width:520px\"> <p class=\"F12 G6 L22 Centent\"><a href=\"" + url + "\" id=\"uu"
                            + isId + "\" onMouseOver=\"UserInfoShowOver(this," + item.userid + "," + item.is_id + ")\" class=\"Blue Fa\">@" + item.nickname + "</a>";
                            original += ShowVerifyImg(item.vi_memberlevel) + "：";
                            original += unescape(item.content) + "</p>";
                            haspic = item.haspic;
                            last = "<div class=\"L30\">";
                            last += "<a href=\"" + contentUrl + "\" class=\"Blue\">" + item.intime + "</a>&nbsp;&nbsp;";
                            last += "<span class=\"G9\">来自<a class=\"Fa\" href=\"" + item.sourceurl + "\">" + item.source + "</a></span> &nbsp; ";
...
var delcontentDialog = "";
//删除博文.by lx on 20120704
function showBlogHtml_Content(spreadId, fromType, userId) {
    ...
    ShowTable += "<input name=\"取消\" type=\"button\" id=\"取消\" value=\"取消\" onclick=\"delcontentDialog.close();\" />";
    ShowTable += "</span></div>";
    ShowTable += "<div class=\"WinBtn  R\"><span>";
    ShowTable += "<input name=\"确定\" type=\"button\" id=\"确定\" value=\"确定\" onclick=\"deleteSpreadInfo_Content(" + spreadId + ",'" + fromType + "'," + userId + ");\"/>";
    ShowTable += " </span></div>";
    ShowTable += "<div class=\"Hr_10\"></div>";
    ShowTable += "</div>";
    ShowTable += "</div>";
...
//弹出删除博文提示框
function showBlogContent_Del(spreadId, fromType, userId) {

    //弹出博文框
    delcontentDialog = $.dialog(
       {

           id: "divShowGroupTrue",
           title: false,
           content: showBlogHtml_Content(spreadId, fromType, userId),
           max: false,
           min: false,
           lock: true,
           cache: false,
   ...
   function deleteSpreadInfo_Content(spreadId, fromType, userId) {
       ...
       $.ajax({
           ...
           success: function(json) {
               ...
               if (dataObj.state == 1) {
                   ...
                   //关闭遮罩
                   delcontentDialog.close();

                   //提示
                   showTipe("博文删除成功", 1);

                   window.setTimeout("funRedirect('/u')", 2000);
...
//获取原创. by lx on 20120716(type:0-原创 1-转发)
        ...
        success: function(json) {
            ...
            if (dataObj.list.length > 0) {
                ...
                $.each(dataObj.list, function(idx, item) {
                    ...
                    var deleteHtml = "<a href=\"javascript:void(0);\" onclick=\"showBlogContent_Del(" + item.is_id + ",'a'," + userid + ");\" class=\"Blue\">删除</a> | ";
                    ...
                    result += item.timestr;
                    result += "&nbsp;";
                    result += "来自<a href=\"" + item.is_url + "\">";
                    result += item.is_name;
                    result += "</a>";

                    $("#iloguserid").val(item.userid);

                    result += "&nbsp;&nbsp;";
                    result += "<a class=\"Blue\" href=\"javascript:void(0);\" onclick=\" ShowPageReport(" + item.is_id + ",'" + item.nickname + "','" + item.face + "','" + item.content + "')\">";
                    result += "举报";
                    result += "</a>";
                    ...
                    if ((item.is_spreadtype == 1 && item.is_isoriginal == 0) || (item.is_spreadtype == 0 && item.is_isoriginal == 0 && item.is_type == 1) || type == 1) {
                        //转发
            ...
            success: function(json) {
                ...
                $.each(dataObj.List, function(idx, item) {
                    ...
                    if (item.is_id != undefined) {
                        ...
                        var iloguserid = $("#iloguserid").val();

                        if (iloguserid == userid) {
                        ...
                        strList += "<a href=\"" + url + "\" >";
                        strList += "<img src=\"images/face/small/" + face + "\" alt=\"" + item.nickname + "\" title=\"" + item.nickname
                        + "\" width=\"30\" height=\"30\" class=\"L\" id=\"miniuser" + item.is_id + "\" onMouseOver=\"UserInfoShowOver(this," + item.userid + "," + item.is_id + ")\" />";
                        strList += "<div class=\"L18 Pl10 L Rbox\">";
                        strList += "<a href=\"" + url + "\" alt=\"" + item.nickname + "\" id=\"nickname" + item.is_id
                        + "\" title=\"" + item.nickname + "\" class=\"Blue\" onMouseOver=\"UserInfoShowOver(this," + item.userid + "," + item.is_id + ")\">";
                        ...
                        strList += "<a href=\"javascript:void(0);\" onclick=\"checkForWard(" + item.userid + "," + item.is_id + "," + item.is_isoriginal + "," + (item.is_spreadnum == "" ? 0 : item.is_spreadnum) + ",'conf');\" class=\"Gray9\">";
            ...
            $.each(dataObj.List, function(idx, item) {
                ...
                if (item.ic_id != undefined) {
                    ...
                    if (userid == item.userid) {
                        ...
                        //                            reportHtml = "<a href=\"javascript:void(0);\" onclick=\"ShowPageReport(" + item.ic_id + ",'" + item.nickname + "','" + item.face + "','" + item.content + "')\" class=\"Gray9\">举报</a> | ";
                        //                        }
                    ...
                    strList += "<a href=\"" + url + "\">";
                    strList += "<img src=\"images/face/small/" + face + "\" alt=\"" + item.nickname + "\" title=\"" + item.nickname
                    + "\" id=\"miniuser" + item.ic_id + "\" onMouseOver=\"UserInfoShowOver(this," + item.userid + "," + item.ic_id + ")\" width=\"30\" height=\"30\" class=\"L\" />";
                    strList += "<div class=\"L18 Pl10 L Rbox\">";
