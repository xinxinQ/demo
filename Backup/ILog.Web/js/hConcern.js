...
$(document).ready(function() {
    ...
    //加载ilog左侧菜单
    funGetleftMenuService(vServiceUrl + "ILogWebLeftMenu.asmx/ILogGetHPersonalLeftMneu", "{MenuLive:'4',hUserID:'" + userid + "'}", "");

    //获取头像及用户统计基本信息
    VipILogPersonalInfo(vServiceUrl + "VipIlogUser.asmx/ILogGetPersonalUserInfoById", userid, 3);

    //获取
    ShowConcernListALL( vServiceUrl + "ILogFollow.asmx/GetList_OtherFan", 1, 45, "", userid);
        ...
        success: function(json) {
            ...
            if (dataObj.UrlState == 1) {
                ...
                if (vType == 3) {
                    ...
                    //显示，他的关注，他的关注数量
                    funShowPageUserCountInfo("" + vServiceUrl + "/VipIlogUser.asmx/ILogGetUserInfoById", otherId, dataObj.nickname);
                }
...
//显示他的关注数
function funShowPageUserCountInfo(url, otherId, nickName) {
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (dataObj.UrlState == 1) {
                ...
                ShowTitle("关注" + nickName + "的人");

                if (dataObj.sex == "female") {

                    $("#hfollow_title").html("<b>她的粉丝</b>");

                }
                else {
                    $("#hfollow_title").html("<b>他的粉丝</b>");

                }
                $("#FollowNikeName").text("已有" + dataObj.fan + "人关注了" + nickName)
        ...
        success: function(json) {
            ...
            $.each(dataObj.ConcernList, function(idx, item) {
                ...
                if (item.RecordID != undefined) {
                    ...
                    strList += " <img src=\"" + (item.face != "" ? "images/face/small/" + item.Face : "" + FaceImagesUrl + "default1.png") 
                    + "\" alt=\"" + item.NickName + "\" class=\"img\"  id=\"user" + item.RecordID + "\" onMouseOver=\"UserInfoShowOver(this," + item.ConcernUserID + "," + item.RecordID + ")\" /></a><br />";
