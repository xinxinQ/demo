...
　
  //加载ilog左侧菜单
  funGetleftMenuService(""+vServiceUrl+"ILogWebLeftMenu.asmx/ILogGetFollowLeftMneu","{MenuLive:'2'}","");

  //关注的数量
  funGetConcern("{}","","spanConcern");
  
　　//获取关注页面内导航
　 funGetFollowMenuService("{MenuLive:'2'}","",0);
...
　  
　  //加载内容
　  ShowFriendsListALL(""+vServiceUrl + "ILogFollow.asmx/GetList_ConcernDouble",1,45,"");
　  
　  
　  //加载下拉事件
...
///获取提醒
//zhangl 20120705
function ShowFriendsPageSecondMenu()
{
...
　　                if(item.UrlState=="0")
　　                {
　　                       $("#Friends_Count").text("互相关注0人");
　　                      
　　                }
　　         }
　　         else
　　         {
　　            $("#Friends_Count").text("互相关注"+item.DoubleConcern+"人");
　　            
　　            if(item.DoubleConcern=="0")
　　            {
　　                   //构建数据
　　                   strList += "<DIV class=\"Hr_20\"></DIV>";
　　                   strList += " <div class=\"G4\">";
　　                   strList += " 抱歉，您还没有互相关注的朋友<br>";
　　                   strList += " <div class=\"Hr_10\"></div> ";       
                       strList += " <div class=\"Line_ilog\"></div>  ";
                       strList += " <DIV class=\"Hr_20\"></DIV> ";
                        
                       $("#Friends_PageList").html(strList); 
　　                       
　　            }
　　         }  
　　     });
...
//keyword：关键字
//zhanl 20120705
function ShowFriendsListALL(servicesUrl,PageCurrent,pagesize,keyword)
{
    //互相关注数量
    ShowFriendsPageSecondMenu();
        ...
        success: function(json) {
            ...
            $.each(dataObj.ConcernList, function(idx, item) {
                ...
                if (item.RecordID != undefined) {
                    ...
                    strList += " <img src=\"" + (item.face != "" ? "images/face/small/" + item.Face : "" + FaceImagesUrl + "default1.png") + "\" alt=\"" + item.NickName + "\" class=\"img L\" id=\"user" + item.RecordID
                   + "\" onMouseOver=\"UserInfoShowOver(this," + item.ConcernUserID + "," + item.RecordID + ");\"  /></a><br />";
                    ...
                    strList += " </div>";
                    if (item.IlogID != "0") {
                        strList += " <p class=\"txt ENG Fa\"> <a href=\"tcont_" + item.ConcernUserID + "_" + item.RecordID + "\" >" + unescape(item.IlogContent) + "(" + item.intime + ")</a></p>"
                    }
...
function sowhpage_ul(PageCurrent,RecordCount,keyword)
{
    ...
    for(var i = RecordCount ; i >= 1 ; i--)
    {
        if(PageCurrent == i)
        {
             strShowPage += "<li id=\"selpage_"+i+"\" ><a href=\"javascript:void(0);\" onclick=\"ShowFriendsListALL('" + vServiceUrl + "ILogFollow.asmx/GetList_ConcernDouble'," + i + ",45,'"+keyword+"')\"  >第<font color=\"red\">" + i + "</font>页</a></li>";
        }
        else
        {
            strShowPage += "<li id=\"selpage_"+i+"\"  ><a href=\"javascript:void(0);\" onclick=\"ShowFriendsListALL('" + vServiceUrl + "ILogFollow.asmx/GetList_ConcernDouble'," + i + ",45,'"+keyword+"')\"  >第" + i + "页</a></li>";
        }      
    }
...
function nextpage(PageCurrent,RecordCount,keyword,pagesort)
{
    ...
    if(pageindex_n <= 2)
    {
        if(RecordCount >= 1)
        {
            ...
            //重新绑定
            ShowFriendsListALL(""+vServiceUrl + "ILogFollow.asmx/GetList_ConcernDouble",pageindex,45,keyword);
        }
    }
    else
    {
        if(RecordCount >= 1)
        {
            ...
            //重新绑定
            ShowFriendsListALL(""+vServiceUrl + "ILogFollow.asmx/GetList_ConcernDouble",pageindex_n,45,keyword);
        }
    }
