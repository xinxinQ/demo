...
namespace Ilog.BLL
{
    public class ILogSpreadhistory
    {
        ...
        {
            DataTable dblSearchList = ILog.DAL.ILogSpreadhistory.GetYesterdaySpreadList(PageCurrent, PageSize, ref RecordCount, type);
        ...
        /// <param name="type">0 每日 1每周</param>
        /// <returns></returns> 
        public static string GetYesterdaySpreadJsonList(int PageCurrent, int PageSize, int type)
        {
            int RecordCount = 0;

            DataTable List = GetYesterdaySpreadList(PageCurrent, PageSize, ref RecordCount,type);
                ...
                if (count > 0)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        if (ooOriginal!=null)
                        {
                            ...
                            strPageList.Append("io_haspic:\"" + ooOriginal.io_haspic + "\",ih_spreadnum:\"" + List.Rows[i]["ih_spreadnum"].ToString() + "\",");
