...
　
  ...
  funGetConcern("{}","","spanConcern");
   ...
   if (followID==0) {
　　//获取关注页面内导航
　      funGetFollowMenuService("{MenuLive:'3'}","",followID);
　  }
　  else{
　      funGetFollowMenuService("{MenuLive:'4'}","",followID);
　  }
　  
...
　//加载全部未关注
  ShowConcernListALL(""+vServiceUrl + "ILogFollow.asmx/GetList_ConcernGoupID",1,45,"",followID);
...
//页面菜单，编辑分组，删除分组;
//zhanlg 20120705
function ShowConcernPageSort(GroupID)
{
  if(GroupID!=0)
  {
    var vcontent=" 排序方式： ";
        ...
        vcontent="<ul>"
        vcontent+="<li class=\"Pl10 L\"><img src=\"http://simg.instrument.com.cn/ilog/blue/images/ico_edie.png\" class=\"L MiNav_img\"  />";
        vcontent+="<a href=\"javascript:void(0)\"  onclick=\"funEditGroup("+GroupID+")\">编辑分组</a></li > ";
        vcontent+="<li class=\"Pl10 L\"><img src=\"http://simg.instrument.com.cn/ilog/blue/images/ico_mistake.png\" class=\"L MiNav_img\"  />";
        vcontent+="<a href=\"javascript:void(0)\"  onclick=\"ShowGroupDelete("+GroupID+")\">删除分组</a></li > ";
       vcontent+="</ul><div class=\"Hr_1\"></div>";
    

        $("#FollowClass_Group").html(vcontent);
    }
    else{
       $("#FollowClass_Group").hide();
    }
}

//编辑分组
//zhangl 20120705
function funEditGroup(GroupID)
{
    ShowGetGroupName(GroupID);
}
...
//keyword：关键字
//zhangl 220120705
function ShowConcernListALL(servicesUrl,PageCurrent,pagesize,keyword,GroupID)
{

    //排序方式 
    ShowConcernPageSort(GroupID)
    ...
    $.ajax({
        ...
        cache: false,
        //请求参数              
        data: "{PageCurrent:'" + PageCurrent + "',PageSize:'" + pagesize + "',keyWord:'" + keyword + "',GroupID:'" + GroupID + "'}",
        //成功           
        success: function(json) {
            ...
            $.each(dataObj.ConcernList, function(idx, item) {
                ...
                if (idx == 1) {
                    ...
                    {
                        sowhpage_ul(PageCurrent, item.RecordCount, keyword, GroupID);   //分页
                ...
                if (item.RecordID != undefined) {
                    ...
                    strList += " <img src=\"" + (item.face != "" ? "images/face/small/" + item.Face : "" + FaceImagesUrl + "default1.png") + "\" alt=\"" + item.NickName + "\" class=\"img\" id=\"user" + item.RecordID + "\" onMouseOver=\"UserInfoShowOver(this," + item.ConcernUserID + "," + item.RecordID + ")\"/></a><br />";
...
//分页控件（加载页面）
//zhangl 20120705
function sowhpage_ul(PageCurrent,RecordCount,keyword,GroupID)
{
    var sowhpage_div = $("#FollowClass_Page");

    var strShowPage = "";
      
    //当然页码等于总也数就隐藏下一页按钮
    strShowPage += "<span class=\"R\" " + (PageCurrent == RecordCount ? "style=\"display:none;\"" : "style=\"display: ;\"") + " id=\"nextbtn\" ><input type=\"image\" src=\"http://simg.instrument.com.cn/ilog/blue/images/btn_page.gif\" onclick=\"nextpage("+PageCurrent+","+RecordCount+",'"+keyword+"',"+GroupID+")\" alt=\"下一页\" /></span>";   
    ...
    for(var i = RecordCount ; i >= 1 ; i--)
    {
        if(PageCurrent == i)
        {
             strShowPage += "<li id=\"selpage_"+i+"\" ><a href=\"javascript:void(0);\" onclick=\"ShowConcernListALL('" + vServiceUrl + "ILogFollow.asmx/GetList_ConcernGoupID'," + i + ",45,'"+keyword+"',"+GroupID+")\"  >第<font color=\"red\">" + i + "</font>页</a></li>";
        }
        else
        {
            strShowPage += "<li id=\"selpage_"+i+"\"  ><a href=\"javascript:void(0);\" onclick=\"ShowConcernListALL('" + vServiceUrl + "ILogFollow.asmx/GetList_ConcernGoupID'," + i + ",45,'"+keyword+"',"+GroupID+")\"  >第" + i + "页</a></li>";
        }      
    }
    ...
    //当前页码小于1就隐藏上一页页码
    strShowPage += "<span class=\"R\" " + (PageCurrent <= 1 ? "style=\"display:none;\"" : "style=\"display: ;\"") + " id=\"upbtn\"  ><input type=\"image\" src=\"http://simg.instrument.com.cn/ilog/blue/images/btn_pageT.gif\" onclick=\"uppage("+PageCurrent+","+RecordCount+",'"+keyword+"',"+GroupID+")\" alt=\"上一页\" /></span>";
...
function nextpage(PageCurrent,RecordCount,keyword,GroupID)
{
    ...
    if(pageindex_n <= 2)
    {
        if(RecordCount >= 1)
        {
            ...
            //重新绑定
            ShowConcernListALL(""+vServiceUrl + "ILogFollow.asmx/GetList_ConcernGoupID",pageindex,45,keyword,GroupID);
        }
    }
    else
    {
        if(RecordCount >= 1)
        {
            ...
            //重新绑定
            ShowConcernListALL(""+vServiceUrl + "ILogFollow.asmx/GetList_ConcernGoupID",pageindex_n,45,keyword,GroupID);
        }
    }
