...
//得到博文列表
function GetIlogList(PageCurrent, pagesize) {
    $.ajax({
        //请求WebService Url
        url: vServiceUrl + "ILog_Spread.asmx/GetTodaySpreadListJsonStr",
        ...
        success: function(json) {
            ...
            $.each(dataObj.List, function(idx, item) {
                ...
                if (item.io_id != undefined) {
                    ...
                    strList += "<span class=\"publish_num Orange\">" + item.io_spreadnum + "</span><div class=\"Hr_3\">";
                    strList += "</div>";
                    strList += "转发</div>";
                    strList += "<a href=\"" + url + "\" >";
                    strList += "<img id=\"user" + item.io_id + "\"  onmouseover=\"UserInfoShowOver(this," + item.userid + "," + item.io_id
                    + ")\"  src=\"/images/face/small/" + item.face + "\" alt=\"" + item.nickname + "\" class=\"L Img\" /></a>";
                    ...
                    strList += "<span class=\"L\"><a href=\"" + contentUrl + "\" class=\"Blue\"  onclick=\"return LoginDiv(16);\">" + item.intime + "</a> &nbsp;来自<a href=\""
                    ...
                    strList += "<a class=\"Blue\" id=\"replyCount" + item.iso_id + "\" href=\"javascript:checkComment(0,'hotc" + item.io_id + "',"
                    + item.iso_id + "," + "1," + (item.io_commentnum == "" ? 0 : item.io_commentnum) + ");\" >评论(" + (item.io_commentnum == "" ? 0 : item.io_commentnum) + ")</a>";
                    if (userid != item.userid) {
                        strList += " | <a class=\"Blue\" href=\"javascript:void(0);\" onclick=\"ShowPageReport(" + item.iso_id + ",'" + item.nickname + "','" + item.face + "','" + item.io_content + "'," + item.userid + "," + item.memberlevel + ")\" >举报</a>";
                    }
                    strList += "</span></div>";

                    strList += "<div id=\"hotc" + item.io_id + "\" ></div>";
